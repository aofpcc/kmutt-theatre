...
$klein->respond("/staff/finance/test", function($request, $response, $service, $app, $validator) {
  ...
  $revenueList = $conn->query("SELECT addDate, sum(amount) as total  FROM G03_FIN_Revenue WHERE  addDate >= '".$startDate." 00:00:00' AND addDate <= '".$endDate." 23:59:59'")->fetch();
  $expensesList = $conn->query("SELECT addDate, sum(amount) as total FROM G03_FIN_Expenses WHERE  addDate >= '".$startDate." 00:00:00' AND addDate <= '".$endDate." 23:59:59'")->fetch();
