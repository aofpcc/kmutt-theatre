...
$klein->respond('POST', '/editprofile/action', function ($request, $response, $service, $app, $validator) {
  ...
  try{
    ...
    $conn->exec("UPDATE G05_Member_profile SET Fname = \"$fst\", Lname = \"$lst\" WHERE MemberID = $userID;");
    $conn->exec("UPDATE G05_Member_address SET Address = \"$adr\", Province = \"$prov\", District = \"$dist\", SubDistrict = \"$Subdist\", ZipCode = \"$post\" WHERE MemberID = $userID;");
    $conn->commit();
  }
...
//get point
$klein->respond('GET', '/g05/test_get_point', function ($request, $response, $service, $app, $validator) {
  $point = $app->point->getPoint("151");
  // use $point above which is the current point of memberID 151 // example jaaaaa
});
