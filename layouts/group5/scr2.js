...
province.onchange = function() {update(province.selectedIndex-1)};

function update(i){
    /*for(var x = 1; x < district.length; x++) {
      district.remove(x);
    }*/
    while(district.length > 1){
      district.remove(district.length - 1);
    }
    for(var j = 1; j < places[i].length; j++)
    {
    var option = document.createElement("OPTION"),
     txt = document.createTextNode(places[i][j][0]);
    option.appendChild(txt);
    option.setAttribute("value",places[i][j][0]);
    district.insertBefore(option,district.lastChild);
    }
}
